...
const PhoneInput = () => {
  ...
  function f() {
    ...
    for (let i = 0; i < NumArray.length; i++) {
      if (
        Select[0] + Select[1] === NumArray[i].code ||
        Select[0] + Select[1] + Select[2] === NumArray[i].code ||
        Select[0] + Select[1] + Select[2] + Select[3] === NumArray[i].code
      ) {
  ...
  return (
    <div>
      <form action="">
        <input className="fff" type="text" value="&#127757;" disabled />
        <input
          className="mainIn"
          type="text"
          maxLength={14}
